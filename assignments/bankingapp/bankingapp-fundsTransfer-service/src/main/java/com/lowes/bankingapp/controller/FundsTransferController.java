...
public class FundsTransferController {
	...
	public ResponseEntity<ResponseMessage> create(@RequestBody @Valid FundsTransfer fundTranObj) throws Exception {	
		...
		fundTranService.create(fundTranObj);

		// Getting current resource path
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
				.buildAndExpand(fundTranObj.getTargetAccoundId()).toUri();

		return ResponseEntity.created(location)
				.body(this.getResponse(fundTranObj.getTargetAccoundId(), "Funds Transfer record Created"));
	}
	...
	@PutMapping("/{id}")
	public ResponseEntity<ResponseMessage> updatefundsTransfer(@RequestBody @Valid FundsTransfer fundTransObj,

			@PathVariable Integer id) {
		fundTransObj.setTargetAccoundId(id);
		fundTranService.update(fundTransObj);

		return ResponseEntity.ok().body(this.getResponse(fundTransObj.getTargetAccoundId(), "Data Updated"));
	}

	@DeleteMapping("/{id}")
	public ResponseEntity<ResponseMessage> deletefundsTransferRecord(@PathVariable Integer id) {
		fundTranService.delete(id);

		ResponseMessage response = getResponse(id, "funds transfer record  deleted");
